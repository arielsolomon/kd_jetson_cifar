...
def precompute_clip_stl10_unlabeled_image_embeddings():
    precompute_clip_stl10_image_embeddings(
        output_dir="data/cifar10/clip/cifar10_unlabeled_image_embeddings",
        dataset_path="../data/cifar/data",
...
def precompute_clip_stl10_train_image_embeddings():
    precompute_clip_stl10_image_embeddings(
        output_dir="data/cifar10/cifar10_train_image_embeddings",
        dataset_path="../data/cifar/data",
        data_split="train"
    )
...
def precompute_clip_stl10_test_image_embeddings():
    precompute_clip_stl10_image_embeddings(
        output_dir="data/cifar10/cifar10_test_image_embeddings",
        dataset_path="../data/cifar/data",
        data_split="test"
    )
...
def precompute_clip_stl10_text_embeddings():
    precompute_clip_text_embeddings(
        output_path="data/cifar10/cifar10_text_embeddings.pt",
...
def get_clip_stl10_text_embeddings():
    return torch.load("data/cifar10/cifar10_text_embeddings.pt")
...
def get_stl10_train_embedding_dataset(transform=None):
    ...
    return EmbeddingDatasetWrapper(
        ...
        embeddings_dir="data/cifar10/cifar10_train_image_embeddings"
    )
...
def get_stl10_test_embedding_dataset(transform=None):
    ...
    return EmbeddingDatasetWrapper(
        ...
        embeddings_dir="data/cifar10/cifar10_test_image_embeddings"
    )
...
def eval_stl10_train_clip_embeddings():
    ...
    with open("data/cifar10/cifar10_train_clip_acc.txt", 'w') as f:
...
def eval_stl10_test_clip_embeddings():
    ...
    accuracy = eval_dataset_clip_embeddings(dataset, text_embeddings)
    with open("data/cifar10/cifar10_test_clip_acc.txt", 'w') as f:
        f.write(f"ACCURACY: {accuracy}")
    return accuracy
...
def train_probe_model_linear():
    train_probe_model(
        output_dir="data/experiments/cifar10/train_probe_model_linear",
...
def train_probe_model_mlp():
    ...
    train_probe_model(
        output_dir="data/experiments/cifar10/train_probe_model_mlp",
        probe_model=model,
        train_dataset=get_stl10_train_embedding_dataset(),
        test_dataset=get_stl10_test_embedding_dataset(),
        learning_rate=3e-4,
...
def train_student_linear_probe(
    ...
    probe_model = nn.Linear(512, len(CIFAR10_LABELS))
    probe_weights = "data/experiments/data/experiments/cifar10/train_probe_model_linear/checkpoint_14.pth"
    if not os.path.exists(probe_weights):
        train_probe_model_linear()
    probe_model.load_state_dict(torch.load("data/experiments/cifar10/train_probe_model_linear/checkpoint_14.pth"))
    train_student_classification_model(
...
def train_resnet18_from_scratch():
    train_model_from_scratch(
        output_dir="data/experiments/cifar10/train_resnet18_from_scratch",
...
def train_resnet18_zero_shot_train_only(f_name):
    train_student_zero_shot(
        ...
        output_dir=f"data/experiments/cifar10/train_resnet18_zero_shot_train_only",
        arch="resnet18",
        temperature=1.,
        train_dataset=get_stl10_train_embedding_dataset()
    )
...
def train_resnet18_zero_shot(f_name):
    train_student_zero_shot(
        ...
        output_dir=f"data/experiments/cifar10/train_resnet18_zero_shot",
        arch="resnet18",
        temperature=1.
    )
...
def train_resnet18_zero_shot_tp5():
    train_student_zero_shot(
        output_dir=f"data/experiments/cifar10/train_resnet18_zero_shot_tp5",
        arch="resnet18",
        temperature=0.5
    )
...
def train_resnet18_zero_shot_t2():
    train_student_zero_shot(
        output_dir=f"data/experiments/cifar10/train_resnet18_zero_shot_t2",
        arch="resnet18",
        temperature=2.0
    )
...
def train_resnet18_linear_probe(f_name):
    train_student_linear_probe(
        ...
        output_dir=f"data/experiments/cifar10/train_resnet18_linear_probe",
        arch="resnet18", 
        temperature=1.
    )
...
def train_resnet18_linear_probe_train_only(f_name):
    train_student_linear_probe(
        ...
        output_dir=f"data/experiments/cifar10/train_resnet18_linear_probe_train_only",
        arch="resnet18", 
        temperature=1.,
        train_dataset=get_stl10_train_embedding_dataset()
    )
...
def train_resnet18_linear_probe_tp5():
    train_student_linear_probe(
        output_dir=f"data/experiments/cifar10/train_resnet18_linear_probe_tp5",
        arch="resnet18", 
        temperature=0.5
    )
...
def train_resnet18_linear_probe_t2():
    train_student_linear_probe(
        output_dir=f"data/experiments/cifar10/train_resnet18_linear_probe_t2",
        arch="resnet18", 
        temperature=2.0
    )
...
def train_resnet34_linear_probe():
    train_student_linear_probe(
        output_dir=f"data/experiments/cifar10/train_resnet34_linear_probe",
        arch="resnet34", 
        temperature=1.
    )
...
def train_resnet50_linear_probe():
    train_student_linear_probe(
        output_dir=f"data/experiments/cifar10/train_resnet50_linear_probe",
        arch="resnet50", 
        temperature=1.
    )
...
def train_resnet18_embedding_text():
    train_embedding_text(f"data/experiments/cifar10/train_resnet18_embedding_text","resnet18")


def eval_resnet18_embedding_text():
    model = timm.create_model("resnet18", num_classes=512)
    model.load_state_dict(torch.load("data/experiments/cifar10/train_resnet18_embedding_text/checkpoint_48.pth"))

    eval_embeddings_model(
        output_dir=f"data/experiments/cifar10/eval_resnet18_embedding_text",
        model=model,
        dataset=get_stl10_test_embedding_dataset(),
        text_embeddings=get_clip_stl10_text_embeddings()
    )
...
def eval_resnet18_embedding_linear():
    ...
    probe_model = nn.Linear(512, len(CIFAR10_LABELS))
    probe_weights = "data/experiments/cifar10/train_probe_model_linear/checkpoint_14.pth"
    if not os.path.exists(probe_weights):
        train_probe_model_linear()
    probe_model.load_state_dict(torch.load(probe_weights))

    eval_embeddings_model(
        output_dir=f"data/experiments/cifar10/eval_resnet18_embedding_linear",
        model=model,
        dataset=get_stl10_test_embedding_dataset(),
        probe_model=probe_model
    )
...
def eval_resnet18_embedding_mlp():
    ...
    probe_weights = "data/experiments/cifar10/train_probe_model_mlp/checkpoint_14.pth"
    if not os.path.exists(probe_weights):
        train_probe_model_mlp()
    probe_model.load_state_dict(torch.load(probe_weights))
    eval_embeddings_model(
        output_dir=f"data/experiments/cifar10/eval_resnet18_embedding_mlp",
        model=model,
        dataset=get_stl10_test_embedding_dataset(),
        probe_model=probe_model
    )
...
def train_resnet18_text_open_images_224_filter90():
    train_student_zero_shot(
        "data/experiments/cifar10/train_resnet18_text_open_images_224_filter90",
        arch="resnet18",
        temperature=1.,
...
def train_resnet18_text_open_images_224_filter90_temp100():
    train_student_zero_shot(
        "data/experiments/cifar10/train_resnet18_text_open_images_224_filter90_temp100",
        arch="resnet18",
        temperature=100.,
        train_dataset=get_stl10_open_images_filtered_dataset_90(),
        test_dataset=get_stl10_test_embedding_dataset(transform=get_open_images_val_transform())
...
def train_resnet18_text_open_images_96_filter90_temp100():
    train_student_zero_shot(
        "data/experiments/cifar10/train_resnet18_text_open_images_96_filter90_temp100",
        arch="resnet18",
        temperature=100.,
